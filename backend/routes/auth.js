...
module.exports = (sql) => {
  ...
  router.post('/users', authMiddleware, adminMiddleware, async (req, res) => {
    ...
    try {
      ...
      const result = await sql`
        ...
        VALUES (${trimmedUsername}, ${hashedPassword}, ${email || null}, ${userRole})
        RETURNING id, username, email, role, created_at
      `;

      console.log(`✅ User created by ${req.user.username}: ${trimmedUsername} (${userRole})`);
      res.status(201).json({
  ...
  router.delete('/users/:id', authMiddleware, adminMiddleware, async (req, res) => {
    ...
    try {
      ...
      console.log(`✅ User deleted by ${req.user.username}: ${deletedUsername}`);
